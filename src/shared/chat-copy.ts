...
const VI: ThreadCopy = {
  ...
  firstVerdict: {
    ...
    typed: "Bạn gõ chứ không chụp — nên khẩu phần là mình đoán. Cứ xem {kcal} là con số áng chừng; nếu bạn biết số gam thì nói, mình sửa.",
...
const ID: ThreadCopy = {
  scripted: {
    ...
    "trial-started": "Uji coba jalan. Tujuh hari, lalu {price} kecuali kamu hentikan — aku ingatkan di hari kelima dan sehari sebelum berakhir, tidak pernah sehari sesudahnya.",
  ...
  firstVerdict: {
    ...
    sodium: "Natrium makanan ini tinggi. Dinilai hanya karena kamu memintanya.",
    satfat: "Lemak jenuh makanan ini tinggi. Dinilai hanya karena kamu memintanya.",
...
const RU: ThreadCopy = {
  scripted: {
    ...
    "camera-primer": "Сначала одно: iOS попросит доступ к камере. Я использую её для тарелки и больше ни для чего — фото хранится вместе с приёмом пищи, чтобы оно было у тебя в дневнике, и стирается вместе с аккаунтом.",
    "fix-prompt": "Скажи, что не так — «риса половина», «без авокадо», «там было 500» — подойдёт любое. Или открой карточку и поправь граммы вручную.",
    "already-in": "Хорошо. Я в чате в любой момент — фото или фраза, и то, и другое записывает приём пищи.",
    "camera-denied": "Нет камеры — не беда. Выбери фото из галереи или просто скажи, что было на тарелке, — вердикт будет в обоих случаях.",
  ...
  firstVerdict: {
    ...
    sodium: "Натрия здесь много. Оценено только по твоей просьбе.",
    satfat: "Насыщенных жиров здесь много. Оценено только по твоей просьбе.",
