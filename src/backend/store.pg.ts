 ...
 * Absent on purpose: `onboarding_content` and `notification_copy` (one row each, id = 1, the same
 * copy for everybody) and `subscribers`, which has no account and must never gain one — see the
 * note on that table and the rule in AGENTS.md.
 *
...
export async function postgresStore(
  ...
  for (const [name, fn] of Object.entries(methods) as [string, unknown][]) {
    ...
    wrapped[name] = async (...args: unknown[]) => {
      // Already inside a scoped call — one store method reaching another. Joining it is right;
      // opening a second transaction would take a second connection and its own snapshot.
      if (active.getStore()) return await call(...args);
