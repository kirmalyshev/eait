...
describe("subscribe", () => {
  ...
  test("a submitted address is NOT on the list until it is confirmed", async () => {
    ...
    const d = deps();
    await subscribe(d, { email: "stranger@example.com", source: "web" });
  ...
  test("a provider that will not send is reported, and the row is left to expire", async () => {
    const d = deps();
    d.mailer.failNext();
    expect(await subscribe(d, { email: "a@example.com", source: "web" }))
  ...
  test("the daily cap is charged on rows added, not on rows we liked", async () => {
    ...
    expect(await subscribe(d, { email: "a@example.com", source: "web" }))
      .toEqual({ ok: true, created: true, pending: true });
    expect(await subscribe(d, { email: "b@example.com", source: "web" }))
      .toEqual({ ok: true, created: true, pending: true });
    expect(await subscribe(d, { email: "c@example.com", source: "web" }))
  ...
  test("the cap is a day wide, not forever", async () => {
    ...
    expect(await subscribe(today, { email: "a@example.com", source: "web" }))
      .toEqual({ ok: true, created: true, pending: true });
    ...
    expect(await subscribe(later, { email: "b@example.com", source: "web" }))
      .toEqual({ ok: true, created: true, pending: true });
  });
  ...
  test("an unconfirmed address is swept once its window has passed", async () => {
    ...
    await subscribe(day0, { email: "never-confirmed@example.com", source: "web" });
    ...
    await subscribe(day8, { email: "someone-else@example.com", source: "web" });
...
describe("unsubscribe", () => {
  ...
  async function joined(d: Deps, email: string): Promise<string> {
    await subscribe(d, { email, source: "web" });
    await confirmSubscription(d, tokenFromLastEmail(d));
