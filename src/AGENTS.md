...
- **Relative meal dates are text-only and set-once.** A text meal's `dayOffset` fixes its day; `applyCorrection` never changes a meal's `date`, so a mis-dated meal is only recoverable via `/delete` + re-entry. The confirm prompt naming the resolved date is the guard against a misparse — do not remove it.
