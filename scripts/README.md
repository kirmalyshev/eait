...
- `eval-meals.ts` — **manual, billed** accuracy eval (issue #6): runs every fixture in the gitignored `eval/` dir (`<name>.jpg` + `<name>.json` with kitchen-scale ground truth, `kcal` required, macros/`total_grams` optional) through one or more OpenRouter models and prints kcal/macro MAE, MAPE, portion error, and run-to-run spread per model. `--models a,b` to A/B, `--runs 3` for variance. Metric logic lives in `src/eval.ts` (unit-tested); this is the I/O + spend shell.
