...
- `service.sh` — run eait as a persistent service, **macOS (launchd) or Linux (systemd user unit)**: `scripts/service.sh {install|start|stop|restart|status|logs|uninstall}`. Generates the plist locally (no machine paths committed); logs → `logs/`.
