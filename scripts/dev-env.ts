...
//
//   bun scripts/dev-env.ts setup          derive, and write .env.worktree and .env
//   bun scripts/dev-env.ts show           print what this worktree resolves to
//   bun scripts/dev-env.ts branch-check   warn when .env.worktree names another branch
//
...
/**
 * The values `.env.worktree` carries, which is the file `scripts/worktree.sh` sources.
 *
...
function setup(root: string): void {
  ...
  writeFileSync(join(root, ".env.worktree"), renderEnv([
    ...
    "",
    "Sourced by scripts/worktree.sh, which supplies slot-0 defaults when this file is absent.",
